    ...
    class MainClass
    {
        ...
        public static void Main(string[] args)
        {
            ...
            for (int i=0; i<n; i++)
            {
                ...
                {
                    if (a % j == 0)         // пробегаемся от 2 до Sqrt(a) чтобы проверить есть ли делители числа а,
                                            // если да то число не прайм, меняем наш бул 
                    {
                        r = 0;
                        break;
                    }
                }
                if (r == 1)     //если число бул добавляем его в лист и увеличиваем каунтер
                {
                    b.Add(a);
                    cnt++;
                }
            }
            Console.WriteLine(cnt);         //выводим кол-во праймов и сами прайм числа
